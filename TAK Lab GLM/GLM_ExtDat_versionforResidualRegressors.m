...
for bl = 1:length(blist)
    ...
    if isempty(params.cellnum) % if there are no active cells (or other cells that meet experimental criteria)
     ...
     for BR = 1:Ops.numBoots  % todo: do this 100-500x ==================================
        ...
        %-----------------------------------------
       [XmatOnes, maskBASE, maskLabelBASE,params] = GLM_makeRegMat(params,Ops,ntfilt,sz,t);
       ...
       % -----------------------------------------
        ...
        for ii = 1:size(ztraces1,1) % loop through neurons in the block.
            ...
            for cv = 1:Ops.RepeatFits % 10x validation - number of tests set in GLM_simple_eventOps
                ...
                elseif Ops.useCorrCell
                    [CorrTraces, params, mask, maskLabel] = GLM_CorrCellReg(Compiled_Table,blist{bl},params.cellnum(ii),ztraces,Ops.CorrType,Ops,...
                        Data.([s]),ResidTraces,params,maskBASE,maskLabelBASE);
            ...
            maskCell{ii} = mask;
            maskLabelCell{ii} = maskLabel;
            clear mask maskLabel
        end
        toc
        % store the data:
        ...
        RidgeData.mask{bl,BR} = maskCell;
        RidgeData.maskLabel{bl,BR} = maskLabelCell;
        if Ops.useAllCells
        ...
        clear Xtest  AllBestFilt AllBestLam  RidgeEst  maskCell maskLabelCell
    ...
    if isempty(RidgeData.TestTrace{bl})
        ...
        for BR = 1:Ops.numBoots
            ...
            for ii =  1:size(testTrace,1)
                 Ws =  RidgeData.Filter{bl,BR}{ii}; % filter from validation/training
                ...
                const = Ws(1);
                mask = RidgeData.mask{bl,BR}{ii};
                maskLabel = RidgeData.maskLabel{bl,BR}{ii};
                start = 1;
                pred = 0;
...
for bl = 1:length(RidgeData.BlockName)
    ...
    for ii =  1:size(testTrace,1)
        ...
        %           figure;
        plot(smooth(T,5),'Color',linecolor_1); hold on
        plot(smooth(pred,5),'Color' ,linecolor_2);
...
%% save figs?
FolderName = 'Z:\Carolyn\2P Imaging data\VIPvsNDNF_response_stimuli_study\GLM\Oct 2023\Loco and corr neurons and mult act stim\RF';
...
FolderName = 'Z:\Carolyn\2P Imaging data\VIPvsNDNF_response_stimuli_study\GLM\Oct 2023\Loco and corr neurons and mult act stim\RF';
cd(FolderName)
savename = string('RidgeRegression_FM.mat');
save(savename, 'RidgeData', '-v7.3');

savename= string('Ops_FM.mat');
