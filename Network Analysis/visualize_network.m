function [fig1, fig2, fig3, fig4] = visualize_network(Correlations,selected_cell,loco,Ops)
...
if isempty(Ops)
    Ops.plot_graph = 1;
    Ops.sig_only = 1; % only plot significant correlations with selected cell
    Ops.use_matched_cell = 1; % block cells = 0, matched cells =1
end
...
if Ops.plot_graph
    ...
    for i=1:n % 1 = Trace Max, 2 = Trace Zero, 3 = Noise, 4 = Signal
        ...
        for m = 1:2
            if i == 1
                fig1 = subplot(1,2,m); hold on
            elseif i == 2
                fig2 = subplot(1,2,m); hold on
            elseif i == 3
                fig3 = subplot(1,2,m); hold on
            elseif i == 4
                fig4 = subplot(1,2,m); hold on;
            end
           ...
           minCorrelation = ceil(abs(min(CorrelationData(exampleCell_ind))*100));
