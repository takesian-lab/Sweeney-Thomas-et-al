function [New_RF, NeighborData, fig1, fig2] = simple_reliability_neighbors(CellDataByStim, StimInfo, plot_figures, Ops)
% This function maximizes detection of a response within a receptive field 
% by measuring response reliability compared to neighboring stimuli 
...
% user data)using trials that are circularly shifted 
...
for v = 1:length(StimInfo.Combined)
    ...
    for a = 1:size(adj_ind) % loop through neighbors
        ...
        Pearsons = corrcoef([trials; neighbor_trials]', 'Rows', 'complete'); %, 'Rows','pairwise'); %Pairwise flag needed to omit nans
        Pearson_AB = Pearsons(1:N1,N1+1:N1+N4); % upper right quadrant compares trials with neighbors
        R = mean(mean(Pearson_AB));
        ...
        %Compute Pearson's correlation for all possible combinations of shuffles
        Pearsons = corrcoef([shifted_trials; shifted_trials_neighbor]','Rows','pairwise'); %Pairwise flag needed to omit nans
        Pearson_AB_s = Pearsons(1:N5,N5+1:N5+N6); % upper right quadrant is Pearsons of shuffled neighbor x neighbor trials, replicated to # of shuffles
        ...
        reliability_shifted = nan(r_shuffles,1);
        ...
        for u = 1:r_shuffles
            ...
            Pearson_AB_shifted = Pearson_AB_s(shuffle_index,shuffle_index_neighbors);
            reliability_shifted(u) = mean(mean(Pearson_AB_shifted));
            index = index+N1;
