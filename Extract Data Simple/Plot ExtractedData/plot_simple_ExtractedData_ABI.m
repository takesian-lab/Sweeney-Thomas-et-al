function plot_simple_ExtractedData_ABI(ExtractedData, UserOps)
...
if ExtractedData.StimInfo.StimProtocol ~= 8
    return;
end
...
disp('Plotting ABI data')

StimData = simple_subset_ExtractedData(ExtractedData, UserOps);

Ops = StimData.SubsetOps; %Ops = StimData.Ops;
...
if isempty(Summary)
    disp('No responsive cells found to plot for ABI')
    return
end
...
%% Plot BestInt and BestRate measures
...
for g = 1:length(Groups)
    ...
    for v = 1:length(variablesToPlot)
        ...
        elseif strcmp(responseToPlot{v}, 'CF_I+20')
            ints = tempStim.CF_I + 20; %not functioning because V2 is rate for ABI and not linear
...
for g = 1:length(Groups)
    ...
    for v = 1:length(variablesToPlot)
        ...
        if strcmp(responseToPlot{v}, 'BF')
            freqs = tempStim.BF;
            %Fill respmat with responses for that intensity row
            for r = 1:size(freqs,1)
                respmatind(r,:) = find(StimInfo.Combined(:,1) == freqs(r));
                respmat(r,:) = norm_response_mat(r,respmatind(r,:));
            end
        else
            error('Response type has not been coded for yet')
