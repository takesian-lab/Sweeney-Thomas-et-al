function plot_simple_ExtractedData_SAMfreq(ExtractedData, UserOps)
% Plot output of SAMfreq stim analysis for ExtractedData
...
%Return if not SAMfreq data
if ExtractedData.StimInfo.StimProtocol ~= 6
    return;
end
...
if isempty(Summary)
    disp('No responsive cells found to plot for SAMfreq')
    return
end
...
variablesToPlot = {'BF', 'BF_fit_halfwidth', 'dprime', 'Sparseness', 'R2'};
responseToPlot = {'BestDepth', 'BestDepth', 'BestDepth', 'BestDepth', 'BestDepth'}; %For heatmap (All depth rows)
...
if Ops.plotCorrelations
    variablesToPlot = {'BF', 'BF_fit_halfwidth', 'BestDepth', 'BestDepth_fit', 'BestDepth_fit_halfwidth', 'DSI', 'dprime', 'Sparseness', 'R2'};
